    ...
    fn replay_one_segment(path: &Path) -> io::Result<(Vec<Record>, Option<u64>)> {
        ...
        loop {
            if pos + 4 > len {
                ...
                return if pos == len { Ok((out, None)) } else { Ok((out, Some(pos))) };
              }
            let mut lb = [0u8; 4];
...
// ---------------------------------------------------------------------------
    ...
    let op = match tag {
         1 => Op::Put { key, value: b[cur..cur + v_len].to_vec() },
         2 => Op::Delete { key },
...
pub fn list_segments(dir: &Path) -> io::Result<Vec<PathBuf>> {
    ...
    for e in std::fs::read_dir(dir)? {
        ...
        if name.to_str().map(|s| s.starts_with("wal-") && s.ends_with(".log")).unwrap_or(false) {
            vec.push(e.path());
       }
   }
    Ok(vec)
...
// ---------------------------------------------------------------------------
    ...
    TABLE.get_or_init(|| {
      ...
      /// IEEE 802.3 CRC-32 (same polynomial as gzip/zlib).
...
mod tests {
    ...
    fn roundtrip_put_and_delete() {
        ...
        w.append(&Record { term: 1, index: 1, op: Op::Put { key: b"a".into(), value: b"1".into() } }).unwrap();
        w.append(&Record { term: 1, index: 2, op: Op::Delete { key: b"a".into() } }).unwrap();
        w.append(&Record { term: 1, index: 3, op: Op::Put { key: b"z".into(), value: b"\x00\x01\xff".into() } }).unwrap();
        let got = recs_clean(&d);
        assert_eq!(got.len(), 3);
        assert_eq!(got[0], Record { term: 1, index: 1, op: Op::Put { key: b"a".into(), value: b"1".into() } });
        assert_eq!(got[1], Record { term: 1, index: 2, op: Op::Delete { key: b"a".into() } });
        assert_eq!(got[2], Record { term: 1, index: 3, op: Op::Put { key: b"z".into(), value: b"\x00\x01\xff".into() } });
    ...
    fn torn_tail_is_dropped_cleanly() {
        ...
        w.append(&Record { term: 1, index: 1, op: Op::Put { key: b"k".into(), value: b"v".into() } }).unwrap();
    ...
    fn rotation_and_replay_order() {
        ...
        for i in 0..50u64 {
            w.append(&Record { term: 1, index: i + 1, op: Op::Put { key: format!("k{i}").into_bytes(), value: i.to_le_bytes().into() } }).unwrap();
           }
        let got = recs_clean(&d);
    ...
    fn crc_detects_bit_flip() {
        ...
        let good = Record { term: 7, index: 42, op: Op::Put { key: b"key".into(), value: b"value".into() } };
