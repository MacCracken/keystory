...
mod tests {
    ...
    fn reader_snapshot_is_isolated_from_later_stores() {
        ...
        assert_eq!(
            *snap, [0u8; 16],
            "frozen reader snapshot must be stable across stores"
        );
        assert_eq!(
            *s.load(),
            [9u8; 16],
            "a fresh loader observes the latest value"
        );
    }
    ...
    fn no_leak_of_replaced_values() {
        ...
        assert_eq!(
            s.live_clones(),
            1,
            "only the current value should be referenced once"
        );
    }
        ...
        for _ in 0..8 {
            ...
            handles.push(thread::spawn(move || {
                while !stop.load(Ordering::Relaxed) {
                    ...
                    assert!(
                        v.iter().all(|&b| b == v[0]),
                        "torn/partial snapshot observed"
                    );
