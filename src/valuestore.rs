...
impl ValueStore {
    pub fn open(dir: impl AsRef<Path>) -> io::Result<ValueStore> {
        let path = PathBuf::from(dir.as_ref().as_os_str()).join(REL_PATH);
    ...
    pub fn compact(&self, live: &BTreeSet<BlobId>) -> io::Result<u64> {
        ...
        let mut out = File::create(&tmp).map_err(|e| io::Error::other(format!("vs tmp: {e}")))?;
        let mut end = 0u64;
        let mut next = 0u64;
        ...
        if let Ok(mut reader) = File::open(&self.path) {
            ...
            while pos + REC_HDR + REC_TRAIL <= data.len() {
                ...
                if live.contains(&id) {
                    let mut b = Vec::with_capacity(REC_HDR + REC_TRAIL + len);
                    b.extend_from_slice(&next.to_be_bytes());
                    b.extend_from_slice(&(len as u64).to_be_bytes());
                    b.extend_from_slice(&data[pos + REC_HDR..pos + REC_HDR + len]);
                    let p2 = b.clone();
...
fn scan_log(inner: &mut Inner) -> io::Result<()> {
    ...
    while pos + REC_HDR + REC_TRAIL <= data.len() {
        ...
        let got = u32::from_be_bytes(data[pos + total - 4..pos + total].try_into().unwrap());
        let pre = data[pos..pos + REC_HDR + len].to_vec();
        if crc32(&pre) != got {
            break; // corrupt: stop
        }
